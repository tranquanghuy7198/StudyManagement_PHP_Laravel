...
<html lang="en">
  <head>
    ...
    <title>Quản lý học tập | Đăng ký học</title>
    <!-- Favicon -->
      ...
      <div class="container">
        ...
        <div id="navbar" class="navbar-collapse collapse">
          <ul id="top-menu" class="nav navbar-nav navbar-right main-nav">
            ...
            @endif
            @if(Auth::check() and $person['who'] != 2)
            <li class="active"><a href="enrol">Đăng ký học</a></li>
            @endif
            <li class="dropdown">
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="title-area">
            <h2 class="title">Đăng ký học tập</h2>
            <p> </p>
          </div>
        </div>
        <div style="padding: 0 0 0 135px" class="sidebar-widget">
          <form action="{{ route('enrolResult') }}" method="post">
            {!! csrf_field() !!}
            <div class="search-group">
              <button type="submit" class="blog-search-btn"><span class="fa fa-plus-square-o"></span></button>
              @if(Auth::user()->who == 1)
              {!! Form::hidden('id', Auth::user()->username) !!}
              @else
              <input style="width: 300px" type="search" name="id" placeholder="Nhập vào MSSV">
              @endif
              <input style="width: 300px" type="search" name="classId" placeholder="Nhập vào mã lớp">
            </div>
          </form>
        </div>
        <table style="width: 900px">
          <thead>
            <tr>
              <th colspan="6">Thông tin đăng ký học tập của sinh viên {{ $student['fullName'] }}</th>
            </tr>
            <tr>
              ...
              <th style="width: 240px">Tên học phần</th>
          ...
          <tbody>
            ...
            <tr>
              ...
              @if(Auth::check() and Auth::user()->who == 3)
              <td class="alignRight">
                {!! Form::open(array('route' => 'deleteEnrolList', 'method' => 'post')) !!}
                {!! Form::hidden('studentId', $row['id']) !!}
                {!! Form::hidden('classId', $row['classId']) !!}
                <button class="ButtonStyle" type="submit" onclick="return confirm('Bạn có chắc muốn xóa bản đăng ký này?')"><i class="fa fa-minus-square-o button delete" style="font-size:20px"></i></button>
                {!! Form::close() !!}
              </td>
              @else
              <td></td>
