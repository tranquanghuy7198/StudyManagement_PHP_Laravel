...
<!-- -------------------------------------------------------------------------------------------------- -->
    ...
    <div class="container">
      <div class="row">
        ...
        <table style="width: 830px">
          <thead>
            <tr>
              <th colspan="6">Thông tin toàn bộ học phần hiện có</th>
            </tr>
            <tr>
              ...
              <th style="width: 50px" class="alignRight" data-target="#addCourseList" data-toggle="modal">
          ...
          <tbody>
            ...
            <tr>
              ...
              {!! Form::open(array('route' => 'updateCourseInfo', 'method' => 'post')) !!}
              {!! csrf_field() !!}
              {!! Form::hidden('courseId', $course[$i]['courseId']) !!}
              <td><input style="width: 380px" class="ButtonStyle" type="text" name="courseName" value="{!! old('courseName', isset($course[$i]) ? $course[$i]['courseName'] : null) !!}"/></td>
              <td>
                <select style="width: 60px" class="ButtonStyle" name="credit">
                  <option value="{!! old('credit', isset($course[$i]) ? $course[$i]['credit'] : null) !!}">{!! old('credit', isset($course[$i]) ? $course[$i]['credit'] : null) !!}</option>
              ...
              {!! Form::close() !!}
              <td class="alignRight">
                {!! Form::open(array('route' => 'deleteCourseList', 'method' => 'post')) !!}
                {!! Form::hidden('courseId', $course[$i]['courseId']) !!}
                <button class="ButtonStyle" type="submit" onclick="return confirm('Bạn có chắc muốn xóa mọi thông tin liên quan tới học phần?')"><i class="fa fa-trash button delete" style="font-size:20px"></i></button>
                {!! Form::close() !!}
              </td>
            </tr>
            @endfor
  ...
  <!-- Start addCourseList modal window -->

  <div aria-hidden="false" role="dialog" tabindex="-1" id="addCourseList" class="modal leread-modal fade in">
    <div class="modal-dialog">
      <!-- Start login section -->
      <div id="login-content" class="modal-content">
        <div class="modal-header">
          <button aria-label="Close" data-dismiss="modal" class="close" type="button"><span aria-hidden="true">×</span></button>
          <h4 class="modal-title"><i class="fa fa-plus"></i>Thêm học phần mới</h4>
        </div>
        <div class="modal-body">
          <form action="{!! route('addCourseList') !!}" method="post">
            {!! csrf_field() !!}
            <div class="form-group">
              <input type="text" name="courseName" placeholder="Tên học phần" class="form-control">
            </div>
            <div class="form-group">
              <input type="text" name="courseId" placeholder="Mã học phần" class="form-control">
            </div>
            <div class="form-group">
              <select name="credit" placeholer="Số tín chỉ" class="form-control">
  ...
  <!-- End addCourseList modal window -->
