    ...
    <div class="container">
      ...
      <div class="col-md-12">
        <div class="title-area">
            <h2 class="title">Lộ trình học sinh viên</h2>
            <p> </p>
        </div>
      </div>
      @if(Auth::user()->who == 3)
      <div style="padding: 0 0 0 170px" class="sidebar-widget">
        <form action="adminFindStudyPath" method="post">
          {!! csrf_field() !!}
          <div class="search-group">
            <button type="submit" class="blog-search-btn"><span class="fa fa-search"></span></button>
            <input style="width: 300px" type="search" name="id" placeholder="Nhập vào MSSV" value="{!! old('id', isset($student) ? $student['id'] : null) !!}">
          </div>
        </form>
      </div>
      @endif
      <table style="width: 830px">
        <thead>
          <tr>
            <th colspan="5">Lộ trình học của sinh viên {{ $student['fullName'] }}</th>
          </tr>
          <tr>
        ...
        <tbody>
          @for($i=0; $i < count($compulsoryCourse); $i++)
          <tr>
            <td>{{ $compulsoryCourse[$i]['courseId'] }}</td>
            <td>{{ $compulsoryCourse[$i]['courseName'] }}</td>
            <td>{{ $compulsoryCourse[$i]['credit'] }}</td>
            <td>{{ isset($evaluate[$i]) ? $evaluate[$i] : null }}</td>
            <td>{{ isset($studyResult[$i]) ? $studyResult[$i] : null }}</td>
          </tr>
          @endfor
        </tbody>
      </table>
