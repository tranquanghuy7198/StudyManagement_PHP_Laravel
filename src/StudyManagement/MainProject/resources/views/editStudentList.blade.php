        ...
        <div class="row">
          <div class="col-md-6 col-sm-6 col-xs-12">
            <div class="single-page-header-left">
              <h2>Quản lý</h2>
              <p>Quản lý toàn bộ hệ thống quản lý học tập sinh viên</p>
            </div>
          </div>
          <div class="col-md-6 col-sm-6 col-xs-12">
...
<!-- -------------------------------------------------------------------------------------------------- -->
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="title-area">
            <h2 class="title">Tìm thông tin sinh viên</h2>
            <p> </p>
          </div>
        </div>
        <table style="width: 1100px">
          <thead>
            <tr>
              <th colspan="7">Thông tin toàn bộ sinh viên trường Đại học Bách Khoa Hà Nội</th>
            </tr>
            <tr>
              <th style="width: 120px">MSSV</th>
              <th style="width: 250px">Họ và tên</th>
              <th style="width: 180px">Ngày sinh</th>
              <th style="width: 250px">Khối lớp</th>
              <th style="width: 200px">Chương trình học</th>
              <th style="width: 50px"></th>
              <th style="width: 50px" class="alignRight" data-target="#addStudentList" data-toggle="modal">
          ...
          <tbody>
            ...
            <tr>
              <td>{{ $row['id'] }}</td>
              {!! Form::open(array('route' => 'updateStudentInfo', 'method' => 'post')) !!}
              {!! csrf_field() !!}
              {!! Form::hidden('id', $row['id']) !!}
              <td><input style="width: 250px" class="ButtonStyle" type="text" name="fullName" value="{!! old('fullName', isset($row) ? $row['fullName'] : null) !!}"/></td>
              <td><input style="width: 180px"  class="ButtonStyle" type="date" name="birthdate" value="{!! old('birthdate', isset($row) ? $row['birthdate'] : null) !!}"/></td>
              <td><input style="width: 250px" class="ButtonStyle" type="text" name="studentGroup" value="{!! old('studentGroup', isset($row) ? $row['studentGroup'] : null) !!}"/></td>
              <td>
                <select style="width: 200px" class="ButtonStyle" name="program">
                  <option value="{!! old('program', isset($row) ? $row['program'] : null) !!}">{!! old('program', isset($row) ? $row['program'] : null) !!}</option>
                  <option disabled="disabled">-------------------------------------</option>
                  <option value="KSTN-CNTT">KSTN-CNTT</option>
                  <option value="KSTN-ĐTVT">KSTN-ĐTVT</option>
                  <option value="KSTN-VLKT">KSTN-VLKT</option>
                  <option value="KSTN-CĐT">KSTN-CĐT</option>
                  <option value="KT22">KT22</option>
                  <option value="KT23">KT23</option>
                  <option value="ICT">ICT</option>
                  <option value="KSCLC-CNTT">KSCLC-CNTT</option>
                </select>
              </td>
              <td class="alignRight">
              ...
              {!! Form::close() !!}
              <td class="alignRight">
                {!! Form::open(array('route' => 'deleteStudentList', 'method' => 'post')) !!}
                {!! Form::hidden('id', $row['id']) !!}
                <button class="ButtonStyle" type="submit" onclick="return confirm('Bạn có chắc muốn xóa mọi thông tin liên quan tới sinh viên?')"><i class="fa fa-user-times button delete" style="font-size:20px"></i></button>
                {!! Form::close() !!}
              </td>
            </tr>
            @endforeach
  ...
  <!-- Start addStudentList modal window -->

  <div aria-hidden="false" role="dialog" tabindex="-1" id="addStudentList" class="modal leread-modal fade in">
    <div class="modal-dialog">
      <!-- Start login section -->
      <div id="login-content" class="modal-content">
        <div class="modal-header">
          <button aria-label="Close" data-dismiss="modal" class="close" type="button"><span aria-hidden="true">×</span></button>
          <h4 class="modal-title"><i class="fa fa-user-plus"></i>Thêm sinh viên mới</h4>
        </div>
        <div class="modal-body">
          <form action="{!! route('addStudentList') !!}" method="post">
            {!! csrf_field() !!}
            <div class="form-group">
              <input type="text" name="fullName" placeholder="Họ và tên" class="form-control">
            </div>
            <div class="form-group">
              <input type="text" name="id" placeholder="Mã số sinh viên" class="form-control">
            </div>
            <div class="form-group">
              <select name="program" placeholer="Chương trình học" class="form-control">
                <option value="KSTN-CNTT">KSTN-CNTT</option>
                <option value="KSTN-ĐTVT">KSTN-ĐTVT</option>
                <option value="KSTN-VLKT">KSTN-VLKT</option>
                <option value="KSTN-CĐT">KSTN-CĐT</option>
  ...
  <!-- End addStudentList modal window -->
