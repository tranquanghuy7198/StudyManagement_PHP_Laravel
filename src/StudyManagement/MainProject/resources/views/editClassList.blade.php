...
<!-- -------------------------------------------------------------------------------------------------- -->
      ...
      <div class="row">
        <div class="col-md-12">
          <div class="title-area">
              <h2 class="title">Tìm thông tin lớp học</h2>
              <p> </p>
          </div>
        </div>
        <table style="width: 1080px">
          <thead>
            <tr>
              <th colspan="7">Thông tin toàn bộ lớp học hiện có</th>
            </tr>
            <tr>
              <th style="width: 100px">Mã lớp</th>
              <th style="width: 330px">Tên học phần</th>
              <th style="width: 150px">Mã học phần</th>
              <th style="width: 250px">Giảng viên</th>
              <th style="width: 150px">Mã giảng viên</th>
              <th style="width: 50px"></th>
              <th style="width: 50px" class="alignRight" data-target="#addClassList" data-toggle="modal">
                <button class="ButtonStyle"><i class="fa fa-plus button add" style="font-size:20px"></i></button>
              </th>
            </tr>
          </thead>
          <tbody>
            ...
            <tr>
              <td>{{ $class[$i]['classId'] }}</td>
              <td>{{ $correspondentCourse[$i]['courseName'] }}</td>
              <td>{{ $class[$i]['courseId'] }}</td>
              {!! Form::open(array('route' => 'updateClassInfo', 'method' => 'post')) !!}
              {!! csrf_field() !!}
              {!! Form::hidden('classId', $class[$i]['classId']) !!}
              <td>
                <select style="width: 240px" class="ButtonStyle" name="teacherId">
                  <option value="{!! old('teacherId', isset($class[$i]) ? $class[$i]['teacherId'] : null) !!}">{!! old('fullName', isset($correspondentTeacher[$i]) ? $correspondentTeacher[$i]['fullName'] : null) !!}</option>
                  <option disabled="disabled">----------</option>
                  @foreach($teacher as $row)
                  <option value="{!! $row['id'] !!}">{!! $row['fullName'] !!} ({!! $row['id'] !!})</option>
                  @endforeach
                </select>
              </td>
              <td>{{ $class[$i]['teacherId'] }}</td>
              <td class="alignRight">
                <button class="ButtonStyle" type="submit"><i class="fa fa-pencil button edit" style="font-size:20px"></i></button>
              </td>
              {!! Form::close() !!}
              <td class="alignRight">
                {!! Form::open(array('route' => 'deleteClassList', 'method' => 'post')) !!}
                {!! Form::hidden('classId', $class[$i]['classId']) !!}
                <button class="ButtonStyle" type="submit" onclick="return confirm('Bạn có chắc muốn xóa mọi thông tin liên quan tới lớp học này?')"><i class="fa fa-trash button delete" style="font-size:20px"></i></button>
                {!! Form::close() !!}
              </td>
            </tr>
            @endfor
  ...
  <!-- Start addClassList modal window -->

  <div aria-hidden="false" role="dialog" tabindex="-1" id="addClassList" class="modal leread-modal fade in">
    <div class="modal-dialog">
      <div id="login-content" class="modal-content">
        <div class="modal-header">
          <button aria-label="Close" data-dismiss="modal" class="close" type="button"><span aria-hidden="true">×</span></button>
          <h4 class="modal-title"><i class="fa fa-plus"></i>Thêm lớp học mới</h4>
        </div>
        <div class="modal-body">
          <form action="{!! route('addClassList') !!}" method="post">
            {!! csrf_field() !!}
            <div class="form-group">
              <input type="text" name="classId" placeholder="Mã lớp" class="form-control">
            </div>
            <div class="form-group">
              <select name="courseId" placeholer="Học phần" class="form-control">
                @foreach($course as $row)
                <option value="{!! $row['courseId'] !!}">{!! $row['courseName'] !!} ({!! $row['courseId'] !!})</option>
                @endforeach
              </select>
            </div>
            <div class="form-group">
              <select name="teacherId" placeholer="Giảng viên" class="form-control">
                @foreach($teacher as $row)
                <option value="{!! $row['id'] !!}">{!! $row['fullName'] !!} ({!! $row['id'] !!})</option>
                @endforeach
              </select>
